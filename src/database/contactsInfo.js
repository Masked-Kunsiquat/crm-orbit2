  ...
  return {
    async getWithContactInfo(id) {
      const [contactRes, infoRes] = await Promise.all([
        ...
        execute('SELECT * FROM contact_info WHERE contact_id = ? ORDER BY is_primary DESC, type ASC;', [id])
      ]);
    ...
    async addContactInfo(contactId, infoData) {
      ...
      if (items.length === 0) {
        throw new DatabaseError('Contact info data is required', 'VALIDATION_ERROR');
      }

      const contactRes = await execute('SELECT id FROM contacts WHERE id = ?;', [contactId]);
      ...
      items.forEach((info) => {
        if (!info || !info.type || !info.value) {
          throw new DatabaseError('Missing required fields: type, value', 'VALIDATION_ERROR');
        }
      ...
      const insertedIds = insertIdx.map((i) => results[i]?.insertId).filter((x) => !!x);
