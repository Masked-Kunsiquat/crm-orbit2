...
describe('createSettingsDB', () => {
  ...
  describe('get', () => {
    ...
    test('throws error for invalid setting key', async () => {
      await expect(settingsDB.get('')).rejects.toMatchObject({
        code: 'VALIDATION_ERROR',
      });
      await expect(settingsDB.get(null)).rejects.toMatchObject({
        code: 'VALIDATION_ERROR',
      });
      await expect(settingsDB.get(123)).rejects.toMatchObject({
        code: 'VALIDATION_ERROR',
      });
    });
  });
  ...
  describe('set', () => {
    test('creates new setting', async () => {
      const result = await settingsDB.set(
        'custom.setting',
        'test value',
        'string'
      );
    ...
    test('validates string values', async () => {
      await expect(
        settingsDB.set('test.key', 123, 'string')
      ).rejects.toMatchObject({ code: 'VALIDATION_ERROR' });
    });

    test('validates number values', async () => {
      await expect(
        settingsDB.set('test.key', 'not a number', 'number')
      ).rejects.toMatchObject({ code: 'VALIDATION_ERROR' });
      await expect(
        settingsDB.set('test.key', NaN, 'number')
      ).rejects.toMatchObject({ code: 'VALIDATION_ERROR' });
    });

    test('validates boolean values', async () => {
      await expect(
        settingsDB.set('test.key', 'not a boolean', 'boolean')
      ).rejects.toMatchObject({ code: 'VALIDATION_ERROR' });
    });

    test('validates JSON values', async () => {
      await expect(
        settingsDB.set('test.key', 'invalid json', 'json')
      ).rejects.toMatchObject({ code: 'VALIDATION_ERROR' });
    ...
    test('throws error for null/undefined values', async () => {
      await expect(
        settingsDB.set('test.key', null, 'string')
      ).rejects.toMatchObject({ code: 'VALIDATION_ERROR' });
      await expect(
        settingsDB.set('test.key', undefined, 'string')
      ).rejects.toMatchObject({ code: 'VALIDATION_ERROR' });
    });

    test('throws error for invalid setting key', async () => {
      await expect(settingsDB.set('', 'value')).rejects.toMatchObject({
        code: 'VALIDATION_ERROR',
      });
      await expect(settingsDB.set(null, 'value')).rejects.toMatchObject({
        code: 'VALIDATION_ERROR',
      });
    });
  });
  ...
  describe('getByCategory', () => {
    test('returns settings for specific category', async () => {
      ...
      expect(
        result.find(s => s.key === 'display.show_avatars' && s.isDefault)
      ).toBeDefined();
      expect(
        result.find(s => s.key === 'display.date_format' && s.isDefault)
      ).toBeDefined();
      expect(
        result.find(s => s.key === 'display.time_format' && s.isDefault)
      ).toBeDefined();
    });
    ...
    test('includes default settings for category', async () => {
      ...
      expect(
        defaultSettings.find(s => s.key === 'display.theme')
      ).toBeDefined();
    });
    ...
    test('throws error for invalid category', async () => {
      await expect(settingsDB.getByCategory('')).rejects.toMatchObject({
        code: 'VALIDATION_ERROR',
      });
      await expect(settingsDB.getByCategory(null)).rejects.toMatchObject({
        code: 'VALIDATION_ERROR',
      });
    });
  });
  ...
  describe('setMultiple', () => {
    ...
    test('throws error for empty or invalid input', async () => {
      await expect(settingsDB.setMultiple([])).rejects.toMatchObject({
        code: 'VALIDATION_ERROR',
      });
      await expect(
        settingsDB.setMultiple([{ key: '', value: 'test' }])
      ).rejects.toMatchObject({ code: 'VALIDATION_ERROR' });
    });
  });
  ...
  describe('reset', () => {
    ...
    test('throws error for setting without default', async () => {
      await expect(settingsDB.reset('unknown.setting')).rejects.toMatchObject({
        code: 'NO_DEFAULT_VALUE',
      });
    });

    test('throws error for invalid setting key', async () => {
      await expect(settingsDB.reset('')).rejects.toMatchObject({
        code: 'VALIDATION_ERROR',
      });
      await expect(settingsDB.reset(null)).rejects.toMatchObject({
        code: 'VALIDATION_ERROR',
      });
    });
  });
  ...
  describe('getAll', () => {
    test('returns all settings with defaults', async () => {
      ...
      expect(
        result.find(s => s.key === 'display.theme' && !s.isDefault)
      ).toBeDefined();
      expect(
        result.find(s => s.key === 'notifications.enabled' && s.isDefault)
      ).toBeDefined();
    });
  ...
  describe('toggle', () => {
    ...
    test('throws error for non-boolean setting', async () => {
      ...
      await expect(settingsDB.toggle('display.theme')).rejects.toMatchObject({
        code: 'INVALID_TYPE_FOR_TOGGLE',
      });
    });

    test('throws error for non-existent setting', async () => {
      await expect(settingsDB.toggle('unknown.setting')).rejects.toMatchObject({
        code: 'SETTING_NOT_FOUND',
      });
    });

    test('throws error for invalid setting key', async () => {
      await expect(settingsDB.toggle('')).rejects.toMatchObject({
        code: 'VALIDATION_ERROR',
      });
      await expect(settingsDB.toggle(null)).rejects.toMatchObject({
        code: 'VALIDATION_ERROR',
      });
    });
  });
  ...
  describe('increment', () => {
    ...
    test('decrements with negative amount', async () => {
      ...
      const result = await settingsDB.increment(
        'display.contacts_per_page',
        -10
      );
    ...
    test('throws error for non-number setting', async () => {
      ...
      await expect(settingsDB.increment('display.theme')).rejects.toMatchObject(
        { code: 'INVALID_TYPE_FOR_INCREMENT' }
      );
    });

    test('throws error for non-existent setting', async () => {
      await expect(
        settingsDB.increment('unknown.setting')
      ).rejects.toMatchObject({ code: 'SETTING_NOT_FOUND' });
    });

    test('throws error for invalid amount', async () => {
      await expect(
        settingsDB.increment('display.contacts_per_page', 'not a number')
      ).rejects.toMatchObject({ code: 'VALIDATION_ERROR' });
      await expect(
        settingsDB.increment('display.contacts_per_page', NaN)
      ).rejects.toMatchObject({ code: 'VALIDATION_ERROR' });
    });

    test('throws error for invalid setting key', async () => {
      await expect(settingsDB.increment('', 5)).rejects.toMatchObject({
        code: 'VALIDATION_ERROR',
      });
      await expect(settingsDB.increment(null, 5)).rejects.toMatchObject({
        code: 'VALIDATION_ERROR',
      });
    });
  });
  ...
  describe('business logic', () => {
    test('maintains setting categories correctly', async () => {
      ...
      expect(
        result.find(s => s.key === 'custom.category.setting')
      ).toBeDefined();
    });
    ...
    test('automatically resolves types from DEFAULT_SETTINGS and JS values', async () => {
      ...
      const overrideSetting = await settingsDB.set(
        'custom.override',
        '456',
        'string'
      );
    ...
    test('handles various is_enabled database formats correctly', async () => {
      ...
      expect(result.find(s => s.key === 'test.enabled_true').isEnabled).toBe(
        true
      );
      expect(result.find(s => s.key === 'test.enabled_t').isEnabled).toBe(true);
      expect(result.find(s => s.key === 'test.enabled_string1').isEnabled).toBe(
        true
      );

      // Should be disabled
      expect(result.find(s => s.key === 'test.disabled_0').isEnabled).toBe(
        false
      );
      expect(result.find(s => s.key === 'test.disabled_false').isEnabled).toBe(
        false
      );
      expect(result.find(s => s.key === 'test.disabled_f').isEnabled).toBe(
        false
      );
      expect(
        result.find(s => s.key === 'test.disabled_string0').isEnabled
      ).toBe(false);
    });
