...
describe('Database Integration Tests', () => {
  ...
  describe('cross-module contact operations', () => {
    test('creates contact with company, categories, and contact info', async () => {
      ...
      await categoriesRelations.addContactToCategory(
        contact.id,
        workCategory.id
      );
      await categoriesRelations.addContactToCategory(
      ...
      expect(
        contactWithInfo.contact_info.find(info => info.type === 'mobile')
      ).toMatchObject({
        value: '+1-555-0123',
        is_primary: 1,
      });
      expect(
        contactWithInfo.contact_info.find(info => info.type === 'email')
      ).toMatchObject({
        value: 'john.doe@techsolutions.com',
        is_primary: 1,
      });
      ...
      const contactCategories =
        await categoriesRelations.getCategoriesForContact(contact.id);
      expect(contactCategories).toHaveLength(2);
      expect(contactCategories.map(cat => cat.name)).toContain(
        `Work Contacts ${timestamp}`
      );
      expect(contactCategories.map(cat => cat.name)).toContain(
        `VIP ${timestamp}`
      );
  ...
  describe('events and reminders integration', () => {
    test('creates events with multiple reminders for a contact', async () => {
      ...
      // 6. Verify reminders
      const birthdayReminders = await eventsReminders.getEventReminders(
        birthdayEvent.id
      );
      expect(birthdayReminders).toHaveLength(2);

      const meetingReminders = await eventsReminders.getEventReminders(
        meetingEvent.id
      );
  ...
  describe('interaction logging and contact updates', () => {
    test('logs interactions and updates last_interaction_at', async () => {
      ...
      expect(updatedContact1.last_interaction_at).not.toBe(
        originalLastInteraction
      );
      ...
      expect(
        new Date(updatedContact2.last_interaction_at).getTime()
      ).toBeGreaterThan(
        new Date(updatedContact1.last_interaction_at).getTime()
      );
  ...
  describe('cascading deletes', () => {
    test('removes all related data when contact is deleted', async () => {
      ...
      const contactWithInfoBefore = await contactsInfo.getWithContactInfo(
        contact.id
      );
      expect(contactWithInfoBefore.contact_info).toHaveLength(1);
      expect(await events.getByContact(contact.id)).toHaveLength(1);
      expect(await interactions.getAll({ contactId: contact.id })).toHaveLength(
        1
      );
      expect(await notes.getByContact(contact.id)).toHaveLength(1);
      expect(await attachments.getByEntity('contact', contact.id)).toHaveLength(
        1
      );
      ...
      const contactWithInfoAfter = await contactsInfo.getWithContactInfo(
        contact.id
      );
      ...
      const contactsInCategory =
        await categoriesRelations.getContactsByCategory(category.id);
  ...
  describe('foreign key constraints', () => {
    test('prevents invalid foreign key relationships', async () => {
      ...
      await expect(
        contactsInfo.addContactInfo(99999, {
          type: 'mobile',
          value: '+1-555-0000',
        })
      ...
      await expect(
        eventsReminders.createReminder({
          event_id: 99999,
      ...
      await expect(
        categoriesRelations.addContactToCategory(contact.id, 99999)
      ).rejects.toThrow(DatabaseError);
    });
  });
  ...
  describe('transaction rollbacks', () => {
    test('rolls back transaction on error', async () => {
      ...
      const initialContactInfo = await contactsInfo.getWithContactInfo(
        contact.id
      );
      ...
      const rolledBackContactInfo = await contactsInfo.getWithContactInfo(
        contact.id
      );
    ...
    test('commits successful transaction', async () => {
      ...
      const committedContactInfo = await contactsInfo.getWithContactInfo(
        contact.id
      );
      expect(committedContactInfo.contact_info).toHaveLength(2);
      expect(
        committedContactInfo.contact_info.map(info => info.value)
      ).toContain('+1-555-3333');
      expect(
        committedContactInfo.contact_info.map(info => info.value)
  ...
  describe('notes with attachments', () => {
    test('creates and retrieves notes with attachments', async () => {
      ...
      const pdfAttachment = noteAttachments.find(
        att => att.file_type === 'document'
      );
      const imageAttachment = noteAttachments.find(
  ...
  describe('settings persistence', () => {
    test('persists settings across sessions', async () => {
      ...
      const notificationSettings =
        await settings.getByCategory('notifications');
      expect(notificationSettings.length).toBeGreaterThanOrEqual(1);

      const enabledSetting = notificationSettings.find(
        s => s.key === 'notifications.enabled'
      );
  ...
  describe('complex cross-module scenarios', () => {
    test('handles complete contact lifecycle with all relationships', async () => {
      ...
      const category1 = await categories.create({
        name: `Clients ${timestamp2}`,
        color: '#00FF00',
        icon: 'user',
      });
      const category2 = await categories.create({
        name: `Priority ${timestamp2}`,
        color: '#FF0000',
        icon: 'star',
      });
      ...
      const contactInteractions = await interactions.getAll({
        contactId: contact.id,
      });
      ...
      const finalContactInfo = await contactsInfo.getWithContactInfo(
        contact.id
      );
      ...
      const contactsInCategory =
        await categoriesRelations.getContactsByCategory(category1.id);
      expect(contactsInCategory).toHaveLength(0);
    });
  });
});
