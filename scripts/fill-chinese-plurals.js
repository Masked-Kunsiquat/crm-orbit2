...
function fillPluralTranslations(filePath) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    // Detect msgid_plural - start of plural block
    ...
    // Fill empty msgstr[0] for plurals (Chinese only needs msgstr[0])
    if (inPluralBlock && line.match(/^msgstr\[0\]\s*""$/)) {
      if (currentMsgid && pluralTranslations[currentMsgid]) {
        const translation = pluralTranslations[currentMsgid];
        if (translation) {
          result.push(`msgstr[0] "${translation}"`);
          fillCount++;
          continue;
        }
      }
    ...
    // End plural block when we hit a non-msgstr line after msgstr entries
    if (inPluralBlock && !line.match(/^msgstr\[/) && line.trim() !== '') {
      inPluralBlock = false;
    }
  ...
  console.log(`✅ 已填充 ${fillCount} 个复数形式翻译`);
  return fillCount;
}
