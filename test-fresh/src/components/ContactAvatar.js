  ...
  useEffect(() => {
    ...
    const loadAvatar = async () => {
      ...
      if (contact?.avatar_attachment_id) {
        try {
          const uri = await loadAvatarFromAttachment(
          ...
          if (mounted) {
            if (uri) {
              ...
              logger.warn(
                'ContactAvatar',
                'Failed to load avatar from attachment',
    ...
    contact?.avatar_attachment_id,
    contact?.avatar_uri,
    loadAvatarFromAttachment,
  ]);

  // Show avatar image if we have a URI
  if (avatarUri) {
    return (
      <Avatar.Image size={size} source={{ uri: avatarUri }} style={style} />
    );
  }
...
export default React.memo(ContactAvatar, (prevProps, nextProps) => {
  return (
    ...
    prevProps.contact?.avatar_attachment_id ===
      nextProps.contact?.avatar_attachment_id &&
