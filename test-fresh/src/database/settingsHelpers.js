...
export function validateSettingValue(key, value, dataType) {
  if (value === null || value === undefined) {
    throw new DatabaseError(
      `Setting value cannot be null or undefined`,
      'VALIDATION_ERROR',
      null,
      { key, value }
    );
    ...
    case 'boolean':
      if (typeof value !== 'boolean') {
        throw new DatabaseError(
          ...
          'VALIDATION_ERROR',
          null,
          { key, value, expectedType: 'boolean' }
        );
      }
      break;
    case 'json':
...
export function castValue(value, dataType) {
  ...
  switch (dataType) {
    ...
    case 'number': {
      ...
      if (isNaN(num)) {
        throw new DatabaseError(
          `Cannot cast value to number: ${value}`,
          'TYPE_CAST_ERROR',
          null,
          { value, dataType }
        );
    ...
    case 'boolean':
      ...
      throw new DatabaseError(
        `Cannot cast value to boolean: ${value}`,
        'TYPE_CAST_ERROR',
        null,
        { value, dataType }
      );
